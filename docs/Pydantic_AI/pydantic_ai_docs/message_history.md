...
### Example: Accessing methods on a `RunResult`
...
### Example: Accessing methods on a `StreamedRunResult`
...
While maintaining conversation state in memory is enough for many applications, you may often want to store the message history of an agent run on disk or in a database. This might be for evals, for sharing data between Python and JavaScript/TypeScript, or any number of other use cases.
...
Since messages are defined by simple dataclasses, you can manually create and manipulate them, e.g., for testing.
